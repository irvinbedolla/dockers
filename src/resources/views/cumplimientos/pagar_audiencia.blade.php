@extends('layouts.app')
@section('title', 'Cumplimiento en Audiencias')
@php
                            ...
                            <!--<a class="btn btn-warning" href="{{ route('todas_audiencias') }}"  onclick=nuevo_poder();> Regresar</a>-->
                                <div class="table-responsive">
                                    <table id="example" class="table table-striped mt-2">
                                        ...
                                        <tbody>
                                            @foreach($cumplimientos as $index => $pago)
                                                <tr>
                                                    ...
                                                    <td class="text-center">
                                                        ...
                                                        @if($pago->estatus == "Pendiente")
                                                            ...
                                                            <a class="btn btn-danger btn-sm text-white" href="{{ route('cumplimiento_rechazar_audiencia', $pago->id) }}" onclick="consultar_estadistica();><i class="bi bi-x-circle me-1"></i> Incumplimiento</a>
                                                            ...
                                                            <form method="POST" action="{{ route('cumplimiento_incomparecencia', $pago->id) }}" style="display:inline;">
                                                                ...
                                                                <button type="submit" class="btn btn-outline-danger btn-sm " onclick="consultar_estadistica();">
                                                                    <i class="bi bi-x-circle me-1"></i> Incomparecencia
                                                                </button>
                                                            </form>
                                                        @endif
...
<div class="modal fade" id="exampleModal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <form id="formCumplimientoModal" class="needs-validation" novalidate method="POST" action="{{ route('pagoA_audiencia') }}">
            @csrf
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $(document).on('click', '.open-modal', function() {
                ...
                if (tipo === 'total') {
                    form.action = "{{ route('audiencia_pagar_total') }}";
                    titulo.innerText = "Descripción para Cumplimiento Total";
                } else {
                    form.action = "{{ route('pagoA_audiencia') }}";
