                ...
                <div class="razon-notificacion"> 
                    ...
                    <p>Siendo las <b>{{ $fechaNotificacion ? $fechaNotificacion->format('H') : '' }} HORAS CON {{ $fechaNotificacion ? $fechaNotificacion->format('i') : '' }} MINUTOS
                        ...
                        @if($citado->quien_atiende == "OTRA PERSONA") {{-- CUANDO ATIENDE OTRA PERSONA CON Y SIN IDENTIFICACIÓN --}}
                            ...
                            Y SEÑAS PARTICULARES: {{$citado->particulares}}. LO ANTERIOR SE HACE DE MANERA APROXIMADA, YA QUE EL SUSCRITO NO ES PERITO EN LA MATERIA</b>.@endif Quien manifiesta que @if($citado->relacion_notificacion=="TRABAJA")<b>OCUPA EL PUESTO DE {{$citado->puesto}}</b>@endif @if($citado->relacion_notificacion=="RESIDE")<b>{{$citado->puesto}}</b>@endif 
